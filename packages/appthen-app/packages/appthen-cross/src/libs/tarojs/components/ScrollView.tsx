...
class ScrollView extends React.PureComponent<ScrollViewProps> {
    ...
    setTimeout(() => {
      ...
      if (props.scrollY && typeof props.scrollTop === 'number') {
        if ('scrollWithAnimation' in props) {
          ...
          if (this.container) this.container.scrollTop = props.scrollTop;
        }
  ...
  render() {
    const { className, onScroll, onScrollToUpper, onScrollToLower, scrollX, scrollY } = this.props;
    ...
    const uperAndLower = () => {
      const { offsetWidth, offsetHeight, scrollLeft, scrollTop, scrollHeight, scrollWidth } =
        this.container || {};
      if (
        onScrollToLower &&
    ...
    const uperAndLowerThrottle = throttle(uperAndLower, 16);
    const _onScroll = (e) => {
